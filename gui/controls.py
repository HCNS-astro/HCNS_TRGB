...
class MockControls(QGroupBox):
    ...
    model with each degradation stage optional: (1) draw from the IDEAL
    broken-power-law LF with a KNOWN injected TRGB and shape (a, b, c);
    (2) add photometric uncertainties from the fitted AST error curve
    sigma(F814W); (3) apply completeness -- keep each star when a uniform
    0-1 draw falls below the fitted completeness curve C(F814W). Loaded as
    the working catalog (SyntheticSession), so the normal panels and fit
    show how the pipeline does on data whose truth is known."""
